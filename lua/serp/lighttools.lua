...
local function replace_chars_for_Name(str)
    -- string should not contain these characters
    str = tostring(str)
    -- str = str:gsub("%,", "-")
...
local function myreplace(source, repl, with)
    -- repl has to be at least 2 characters long
    if repl == "." then
        repl = "%" .. repl
    end
...
local function SortFnBigToSmall(a, b)
    return a > b
end
...
local function _TableToString(_table)
    ...
    for k, v in pairs(getkvtable(_table)) do
        --getkvtable to cycle through
        if #_string > 1 then
            _string = _string .. ","
        end
        if type(_table[k]) == 'table' then
            -- use the original table for direct values
...
local function argstotext(sep, ...)
    ...
    for _, arg in pairs(args) do
        -- pairs instead of ipairs to also allow a value of nil
...
local function WasNewGameJustStarted()
    -- call it within the first 30 seconds of a new game for it to work
...
local function t_FnViaTextEmbed(PID)
    if PID == nil or PID == ts.Participants.GetGetCurrentParticipantID() then
        ...
        if TaskString ~= oldname then
            -- only the correct Peer (who hit the button)
...
local function GetGameObjectPath(OID, path)
    ...
    if OID ~= nil and OID ~= 0 then
        if type(OID) == "number" then
            -- GetGameObject can not handle other than number
...
local function GetActiveQuestInstances(DescriptionTextGUID, firstfound, startfromID, logloop, maxemptycount)
    ...
    while true do
        ...
        if not IsActive and not HasEnded and StoryText == 0 and DescriptionText == 0 and TimeLeft == 0 then
            -- most likely reached the end of quests
            abortcount = abortcount + 1
        else
            abortcount = 0
        end
        if abortcount >= maxemptycount then
            -- only after the last 20 quests were empty we can be sure enough... (ok that much "empty" quests usually only happen on testing much, not sure exactly what causes them... I think A7_QuestSubQuest)
            break
        end
...
local function IsUserdataValid(userdata, OID)
    ...
    if not name:find("oid") then
        -- eg "GameObject unassigned" or "Invalid gameObject id"
        return nil
    end
    return userdata
        ...
        if userdata ~= nil then
            ...
            if success then
                if PropertyName == nil then
                    return nil
                elseif not PropertyName:find("Invalid") and not PropertyName:find("unassigned") then
                    -- "Invalid gameObject id" userdata eg. because no longer filled, was never valid or called in wrong session. "GameObject, unassigned" if we using an OID from another session
                    return true
                else
                    return nil
                end
...
local function GetActiveMods()
    ...
    for i, line in ipairs(modloaderlines) do
        if i >= latestmodloadingline then
            if string.find(line, " %[info%] Load %[") then
                -- successfully loaded
