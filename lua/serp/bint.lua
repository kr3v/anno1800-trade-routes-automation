g_bint_Library051 = nil -- a global variable so we can acces it without "require", because this does not seem to find the files in anno if we have it in modfolder
...
local function newmodule(bits, wordbits)
    ...
    function bint.fromle(buffer)
        ...
        if #buffer > BINT_BYTES then
            -- trim extra bytes from the right
            buffer = buffer:sub(1, BINT_BYTES)
        elseif #buffer < BINT_BYTES then
            -- add missing bytes to the right
    ...
    function bint.frombe(buffer)
        assert(type(buffer) == 'string', 'buffer is not a string')
        if #buffer > BINT_BYTES then
            -- trim extra bytes from the left
            buffer = buffer:sub(-BINT_BYTES, #buffer)
        elseif #buffer < BINT_BYTES then
            -- add missing bytes to the left
    ...
    bint.is_bint = true -- so we have sth to check if the table is bint object or not...

    return bint

end

g_bint_Library051 = newmodule
